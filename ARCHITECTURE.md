...
**同步从今天倒着往 2016 年拉，够用就解锁**：覆盖到最近两年（约 35 分钟）即可开始查询，更早的历史后台继续补。
`/api/plan`、`/api/run` 先检查 `DataSync.status()`，最小可用区间还没到就返回 `data_not_ready`（附进度），
前端输入框不可用；到了之后照常查询，页面标注「历史已补到 YYYY-MM，仍在继续」。

这样做的理由：实测全量要约 3 小时，让用户装完干等三小时才能问第一个问题是不可接受的；而高频问题问的都是"最近"，
...
零失败、稳定 50 次/分钟，10 路 61 次/分钟，12 路开始返回 `code=429 请勿使用过多线程，连接超限`。
**取 8 路并发，全量约 3 小时**；单月面板 9.6 MB，十年磁盘约 1.2 GB。
