...
POST /api/run/{run_id}/narrative
  ...
  说明：卡下面的小结（DESIGN.md §1.6）。卡先出（结果里 narrate=true 表示要占小结的位置），页面再调这里，
        大模型写一次十几秒到一分钟。写过就给存下的那段，不重写。text 为空时页面上这一块不显示。
...
└── narratives/n<run_id>.json         # 卡下面的小结：那段话和写它的每次大模型调用
```
...
| 卡下面的小结 | 交给大模型的是原话和卡上的文字；阿拉伯数字、全角数字、百分号、汉字写的数值都拦下并重试一次，两次不过不给话；说哪一段的「两年」「上半年」不拦；没调用输出工具原样再问（`tests/test_narrator.py`）。真实数据 + 假大模型：卡先出、不等小结；小结单独取，写了数字重试后存下，写过就不重写；不要小结的卡取小结报 400；没配大模型卡照样出（`tests/contract/test_plan_api.py`、`test_run_api.py`）。存取、只能挂在运行上（`tests/contract/test_store.py`） |
