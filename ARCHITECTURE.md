...
api ──→ DataSync.status()：首次同步未完成 → 直接返回 data_not_ready（附同步进度）
api ──→ 组装上下文：
...
api ──→ DataSync.status()：首次同步未完成 → data_not_ready
...
| 场景 | 行为 |
|---|---|
| 首次同步 | 用户点击「同步历史数据」，后台下载 2016 年至今的全部数据，页面显示进度 |
| 拉取方式 | 股票类按交易日拉（一次返回当天全部股票）；`sw_daily` 按行业、`tdx_daily` 按板块、`stock_st` 按日期区间，都比按交易日省得多 |
| 完整日判定 | manifest 按天记录每个必需接口是否已落盘。**只有全部落盘才算完整日**，不完整的当天不对外可见 |
| 原子写入 | Parquet 先写临时文件再改名，避免读到写了一半的文件 |
| 断点续传 | 以月为单位落盘，每完成一个月记入 manifest；中途关闭程序，下次从断点继续 |
| 增量同步 | 之后每次只补"本地最新日期 → 今天"的缺口 |
| 限流与失败 | 按接口限速；失败自动重试，连续失败则暂停并在页面显示原因 |
| 并发 | 同一时间只运行一个同步任务 |

**首次同步完成前，系统不接受任何查询**：`/api/plan`、`/api/run` 先检查 `DataSync.status()`，未完成则返回
`data_not_ready`（附进度），前端输入框不可用。P0 不判断具体问题需要哪些数据，只看"首次同步是否完成"这一个状态。
...
│   └── manifest.json                  # 数据来源、覆盖范围、行数、哈希、同步时间、能力探测结果
└── store/                             # 归 store 模块所有，见 §7
```

`manifest.json` 是本地数据的"户口本"：数据来源、每个数据集的覆盖范围、**每个交易日各必需接口是否已落盘**、
行数与哈希、同步时间、能力探测结果。六个用途：断点续传、增量同步、完整日判定、页面显示数据截止日、
能力开关、出问题时可追溯。

- 日频数据按月分文件：已完成的月份不再改动，增量同步只新增或补全当月文件
